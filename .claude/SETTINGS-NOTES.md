...
| Omitted | Reason |
|---|---|
| `enabledPlugins`, `extraKnownMarketplaces` | **Removed on 2026-09-20; not in this file.** Verified that day: in a cloud session they installed nothing, at fresh startup as well as mid-session — the reconcile ran and reported no failure. Declaring them here stated a capability this repository does not have. They are correct locally, so they moved to `profile/plugins.json` and are applied by `local/bootstrap.sh --with-plugins`. A second row here once described the marketplace list as though it were still present, twenty lines below this record of its removal; `tools/_claim_scan.py` exists because that kind of leftover is what keeps happening. |
