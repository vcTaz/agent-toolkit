...
- **The repository root is also a plugin.** `.claude-plugin/` makes it the plugin
  ...
  holds their frontmatter to flat `key: value` lines with a fixed set of keys, and refuses
  any `!` beside a backtick in them, which is how Claude Code's inline shell starts.
